 ...
 *
 * Receives a provider-neutral PaymentSessionRequest
...
class StripePaymentProvider(
    ...
    override fun createPaymentSession(
        ...
        if (secretKey.isNullOrBlank()) {
            throw ServiceException(
                ...
                description =
                    "STRIPE_SECRET_KEY is required to create a Stripe checkout session"
            )
        }
        ...
        if (response.statusCode() !in 200..299) {
            throw ServiceException(
                errorCode =
                    ServiceErrorCode.CheckoutCreationFailed,
                description =
                    "Stripe checkout request failed with status " +
        ...
        val body = runCatching {
            ...
            throw ServiceException(
                errorCode =
                    ServiceErrorCode.CheckoutCreationFailed,
                description =
                    "Stripe checkout response was not valid JSON"
            )
        }
        ...
        if (id.isNullOrBlank()) {
            throw ServiceException(
                errorCode =
                    ServiceErrorCode.CheckoutCreationFailed,
                description =
                    "Stripe checkout response did not include a session id"
            )
        }
    ...
    private fun PaymentSessionRequest.toStripeFormBody(): String {
    ...
    private companion object {
        const val DEFAULT_CHECKOUT_API_URL =
